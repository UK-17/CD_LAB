...
int main(){
	FILE *fa;
	char types[][20]={"No Token", "Arithmetic Operator", "Relational Operator", "Logical Operator", "Special Symbol", "Keyword", "Number Constant", "String Literal", "Identifier", "Shorthand Operator", "Assignment Operator"};
	int l=1, c=0;
	...
	do{
		...
		if(temp && temp->type!=0)
			printf("< '%6s',%2d,%2d,%6s >\n", temp->ln, temp->row, temp->col+1, types[temp->type]);
	}while(temp!=NULL);
}
